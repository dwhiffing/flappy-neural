...
class ActivationFunction {
  ...
  dfunc: (n: number) => number
  constructor(func: (n: number) => number, dfunc: (n: number) => number) {
    this.func = func
    this.dfunc = dfunc
  }
}
...
export class NeuralNetwork {
  ...
  constructor(
    input_nodes: NeuralNetwork | number,
    hidden_nodes?: number,
    output_nodes?: number,
  ) {
    if (input_nodes instanceof NeuralNetwork) {
      ...
      this.hidden_nodes = hidden_nodes ?? 0
      this.output_nodes = output_nodes ?? 0
